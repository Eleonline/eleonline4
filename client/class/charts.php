...
//====================================
function InsertChart( $flash_file, $library_path, $php_source, $width=470, $height=350, $bg_color="ffffff", $transparent=false, $license=null ){
	
	$php_source=urlencode($php_source);
	$library_path=urlencode($library_path);
	if(isset($_SERVER['HTTPS'])) $protocol = (strtolower($_SERVER['HTTPS']) != 'on')? 'http': 'https';
	else $protocol='http';
 	$x=$_SESSION['max'];
	...
	$html.="width='".$width."' height='".$height."' id='charts$x' />";
	$u=(strpos ($flash_file,"?")===false)? "?" : ((substr($flash_file, -1)==="&")? "":"&");
