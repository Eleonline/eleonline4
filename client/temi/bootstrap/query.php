...
global $dbi,$prefix,$id_comune,$id_cons_gen,$id_cons;
	...
	if($id_cons_pred)
	{
		$sql="SELECT id_cons_gen,id_cons,proiezione FROM ".$prefix."_ele_cons_comune where id_cons='$id_cons_pred' and chiusa<'2'";
		$sth = $dbi->prepare("$sql");
		$sth->execute();
		$row = $sth->fetchAll();
		$id_cons_gen=$row[0][0];
	...
	$sql="SELECT id_cons_gen,id_cons,proiezione FROM ".$prefix."_ele_cons_comune where id_comune='$id_comune' and chiusa<'2' order by id_cons desc limit 0,1";
	$sth = $dbi->prepare("$sql");
	$sth->execute();
	$row = $sth->fetchAll();
	$id_cons_gen=$row[0][0];
...
{
global $dbi,$prefix,$id_comune;
	$sql="SELECT t1.descrizione, t1.id_cons_gen, t1.data_inizio,t3.id_cons,t1.link_trasparenza FROM ".$prefix."_ele_consultazione as t1,".$prefix."_ele_cons_comune as t3 where t1.id_cons_gen=t3.id_cons_gen and t3.id_comune=$id_comune and t3.chiusa<'2' order by t1.data_inizio desc";
	$sth = $dbi->prepare("$sql");
	$sth->execute();	
	$row = $sth->fetchAll();
	return($row);	
...
{
global $dbi,$prefix,$id_comune;
/*	$sql="SELECT max(data_inizio) FROM `soraldo_ele_consultazione`"; 	
	$sth = $dbi->prepare("$sql");
	$sth->execute();	
	list($datainizio)=$sth->fetch(PDO::FETCH_NUM);; */
...
function elenco_liste()
{
	...
	$sql="SELECT id_lista,num_gruppo,num_lista,descrizione,0,link_trasparenza FROM ".$prefix."_ele_lista where id_cons='$id_cons' $filtro order by num_lista";
	$sth = $dbi->prepare("$sql");
	$sth->execute();
	$row = $sth->fetchAll();
	return($row);	
...
function elenco_sezioni($idsede)
{
	global $id_cons,$prefix,$dbi,$circo,$idcirc;
	if($circo){
		...
		$filtrocirco='';
	if($idsede>0) 
		$filtrosede="and id_sede='$idsede'";
	else
		$filtrosede='';
	$sql="select * from ".$prefix."_ele_sezione where id_cons='$id_cons' $filtrocirco $filtrosede order by num_sez";
	$sth = $dbi->prepare("$sql");
	$sth->execute();
	$row = $sth->fetchAll();
	return($row);	
...
global $dbi,$prefix,$id_cons_gen;
if(!$id) $id=$id_cons_gen;
# tipo_cons 	descrizione 	lingua 	genere 	voto_g 	voto_l 	voto_c 	circo 	
