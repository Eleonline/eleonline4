...
$dati = voti_gruppo_graf();
# t2.num_gruppo,t2.descrizione,sum(t1.voti)
$descr = [];
$perc = [];
$voti = [];
$tot = 0;
...
foreach ($dati as $val) {
    $descr[] = $val[1];
    $perc[] = number_format($val[2] / $tot * 100, 2);
    $voti[] = number_format($val[2],0,'','.');
    $i++;
}
$labels = $descr; // Nomi di Gruppo 
$voti_percentuali = $perc; // Percentuali di Gruppo 
$voti_lista = $voti; // Aggiungi i voti per ciascun gruppo

// Palette di colori per le barre (fino a 30 gruppi)
$palette = [
...
<style>
    #chartContainer {
        width: 100%;
        height: <?php echo min(300 + count($labels) * 30, 900); ?>px; /* L'altezza si adatta al numero di gruppi */
        margin: 0 auto;
    }
    #legendContainer {
        text-align: center;
...
<div class="container">
   <div class="row text-center">
        <h4 class="fw-semibold text-primary mobile-expanded mt-2">Voti per <?php echo _GRUPPO; ?> <?php /* ?>
		<?php /*if ($tipo_cons==3 or $tipo_cons==5) echo 
			elseif ($tipo_cons==1 or $tipo_cons==4 or $tipo_cons==9 or $tipo_cons==12 or $tipo_cons==13 or $tipo_cons==17 ) echo "Candidati a Presidente";
			else echo "Raggruppamenti";
			echo "<br>"; 
		?>*/ ?>
		</h4>
    </div> 
<div class="table-responsive overflow-x">
	<table class="table  mb-0">
		<thead class="title-content">
			<tr>
				<th><?php echo _GRUPPO; /*if ($tipo_cons==3 or $tipo_cons==5) echo "Candidati a Sindaco";
			elseif ($tipo_cons==1 or $tipo_cons==4 or $tipo_cons==9 or $tipo_cons==12 or $tipo_cons==13 or $tipo_cons==17 ) echo "Candidati a Presidente";
			else echo "Raggruppamenti"; */
			echo "<br>";
		?></th> 
		<?php $oplink="graf_gruppo"; $infolink=""; include 'temi/bootstrap/pagine/tab_link_opendata.php'; ?>	
