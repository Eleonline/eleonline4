<?php
	...
	# Blocco opendata
	/*$linkopendata="modules.php?name=Elezioni&op=gruppo_sezione&id_cons_gen=$id_cons_gen&id_comune=$id_comune&csv=1&min=1&minsez=1&offsetsez=$totsez&id_lista=";
	$linkopendatapdf="modules.php?name=Elezioni&op=gruppo_sezione&id_cons_gen=$id_cons_gen&id_comune=$id_comune&csv=1&id_lista=";
	$nosez=0;
	...
	<table class="table tablescroll table-striped table-bordered table-sm align-middle ">
		<thead class="primary-bg-c1 text-end">
			<tr>
				...
				<?php } if(!isset($controllo)) $controllo=$val['num_sez']+1; ?>
		...
		foreach($row as $key=>$val){
			...
			foreach ($sezioni as $k=>$i)$ar[$numgruppo][$i['num_sez']]=0;
		}
		$row=voti_gruppo($tab); 
		$righe=count($row);
		foreach($row as $key=>$val){
		...
		<tbody>
			...
			$ar["bianchi"][0]="Schede Bianchi";
			$ar["contestati"][0]="Voti Contestati";
			foreach ($sezioni as $k=>$val2) {
				$ar["validi"][$val2['num_sez']]="0";
				$ar["nulli"][$val2['num_sez']]="0";
				$ar["bianchi"][$val2['num_sez']]="0";
				$ar["contestati"][$val2['num_sez']]="0";
			}
				$totv=0;
				$totn=0;
				$totb=0;
			...
			foreach($sezioni as $key=>$val) {
				$ar["validi"][$val[3]]=$val[6];
				$ar["nulli"][$val[3]]=$val[7];
				$ar["bianchi"][$val[3]]=$val[8];
				$ar["contestati"][$val[3]]=$val[9];
				$totv+=$val[6];
				$totn+=$val[7];
				$totb+=$val[8];
				$totc+=$val[9];
		...
		<tfoot>
			<tr class="primary-bg-c4 white-color align-middle text-start">
				...
				<?php foreach($sezioni as $key=>$val) {?>
					<th class="text-center">
						<?php echo number_format($ar["validi"][$val['num_sez']],0,'','.') ?>
					</th>
				<?php } ?>
				<th class="text-center">
			...
			<tr class="primary-bg-c4 white-color align-middle text-start">
				...
				<?php  foreach($sezioni as $key=>$val) {?>
					 <th class="text-center">
						<?php  echo number_format($ar["nulli"][$val['num_sez']],0,'','.') ?>
					 </th>
				<?php } ?>
				<th class="text-center"> 
			...
			<tr class="primary-bg-c4 white-color align-middle text-start">
				...
				<?php foreach($sezioni as $key=>$val) { ?>
					<th class="text-center"> <?php echo number_format($ar["bianchi"][$val['num_sez']],0,'','.') ?></th>
				<?php } ?>
			...
			<tr class="primary-bg-c4 white-color align-middle text-start">
				...
				<?php foreach($sezioni as $key=>$val) {?>
					<th class="text-center"> <?php  echo number_format($ar["contestati"][$val['num_sez']],0,'','.') ?></th>
				<?php } ?>
