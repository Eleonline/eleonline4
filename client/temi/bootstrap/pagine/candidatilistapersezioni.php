<?php
	...
	# Blocco opendata
	/*$linkopendata="modules.php?name=Elezioni&op=candidato_sezione&minsez=1&offsetsez=$totsez&id_cons_gen=$id_cons_gen&id_comune=$id_comune$cirpar&csv=1&id_lista=$id_lista";
	$linkopendatapdf="modules.php?name=Elezioni&op=candidato_sezione&id_cons_gen=$id_cons_gen&id_comune=$id_comune$cirpar&csv=1&id_lista=$id_lista";
	$nosez=0;
...
<div class="container pb-2">
  <label for="defaultSelect">Seleziona Lista</label>
	...
	foreach($elencoliste as $campo=>$val) {
		...
		<option <?php echo $sel; ?> value="modules.php?op=15&name=Elezioni&id_comune=<?php echo $id_comune.$cirpar;?>&id_cons_gen=<?php echo $id_cons_gen;?>&file=index&num_lista=<?php echo $val[2];?>"><?php echo $val[2].") ".$val[3];?></option>
	<?php } ?>
	</select>
</div>
<?php
	...
	$offsetmax=count($row);
?>
<?php $linkopendata="modules.php?name=Elezioni&op=candidato_sezione&minsez=1&offsetsez=$totsez&id_cons_gen=$id_cons_gen&id_comune=$id_comune$cirpar&csv=1&id_lista=$id_lista&min=1&offset=$offsetmax";
	$linkopendatapdf="modules.php?name=Elezioni&op=candidato_sezione&id_cons_gen=$id_cons_gen&id_comune=$id_comune$cirpar&csv=1&id_lista=$id_lista&min=1&offset=$offsetmax";
	$nosez=0;
...
<div class="table-responsive overflow-x">
	<table class="table  mb-0">
		<thead class="title-content">
			<tr>
				<th><?php echo _CANDIDATI." per sezione";?></th> 
	...
	<table class="table tablescroll table-striped table-bordered table-sm align-middle text-end">
		<thead class="primary-bg-c1 align-middle">
			<tr>
				...
				<?php } if(!isset($controllo) and isset($val['num_sez'])) $controllo=$val['num_sez']+1; ?>
		...
		foreach($row as $key=>$val){
			$numcand=$val[2]; 
			$ar[$numcand][0]=$val[3];
			foreach ($sezioni as $k=>$i) $ar[$numcand][$i['num_sez']]=0;
		}
		$row=voti_candidati($num_lista); 
		$righe=count($row);
		foreach($row as $key=>$val){
			$numcand=$val[2]; 
			$sezione=$val[1];
			...
			if(!isset($ar["$numcand"][$controllo]))
				foreach ($sezioni as $k=>$val2) 
					$ar["$numcand"][$val2['num_sez']]="";
			$ar["$numcand"][$val[1]]=$val[4];
