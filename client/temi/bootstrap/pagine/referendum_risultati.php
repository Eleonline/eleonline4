...
$c= 0;
$i= 0;
// Funzione per ottenere l'immagine predefinita dal config_colori_quesiti.php
function getImmaginePredefinita($chiave_colore) {
    global $coloriQuesiti;
    if (isset($coloriQuesiti[$chiave_colore])) {
        return $coloriQuesiti[$chiave_colore]['immagine'];
    }
...
// mettere il valore del colore $c preso dal db
$c= $val['id_colore'] ;
$coloreSfondo = isset($coloriQuesiti[$c]['colore']) ? $coloriQuesiti[$c]['colore'] : '0';
?>
...
// Verifica immagine personalizzata
if (presenza_immagine('gruppo', $val['id_gruppo'])) {
    $img_url = "modules.php?name=Elezioni&file=foto&id_gruppo=" . $val['id_gruppo'];
} else {
    // Se non c'è immagine personalizzata, prova con immagine predefinita se valida
    $colore = $c;  // oppure $val['colore'] se definito
    $immagine_predefinita = getImmaginePredefinita($colore);

    if (!empty($immagine_predefinita)) {
        $percorso_file = 'temi/bootstrap/pagine/' . $immagine_predefinita;
        if (file_exists($percorso_file)) {
            $img_url = $percorso_file;
        }
    }
}
?>
				...
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					<?php if(isset($si[$val['id_gruppo']])) { ?>
						<table class="dati">
							<tbody>
								<tr>
									<th id="hsi<?php echo $val['id_gruppo'];?>" scope="row" class="dati_referendum">SI</th>
									<th id="hno<?php echo $val['id_gruppo'];?>" scope="row" class="dati_referendum">NO</th>
								</tr>
								<tr>
